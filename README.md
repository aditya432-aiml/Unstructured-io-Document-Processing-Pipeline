...
2. Chunks content at heading/section boundaries using `chunk_by_title`
3. Normalizes chunked elements into a stable, flat dictionary schema
...
└────────────┬────────────┘
             ...
             ▼
┌─────────────────────────┐
│  Step 2 — Chunking      │  chunk_by_title (max 1200 chars)
└────────────┬────────────┘
             │
             ▼
┌─────────────────────────┐
│  Step 3 — Normalization │  Flatten to stable dict schema
└────────────┬────────────┘
             │
             ▼
┌─────────────────────────┐
...
├── single_pdf_parsing_pipeline.py               # ← Single PDF workflow
...
### Option A — Parallel multi-PDF

Processes all PDFs under `Training_Data/` using `ProcessPoolExecutor` with up to 4 workers. Each worker initialises its own model and ChromaDB client. Includes a skip check — already-processed PDFs (matched by `source_file` metadata) are not re-embedded.

```bash
python multi_pdf_parallel_processing_pipeline.py
```

- Collection: `new_multi_pdf_pipeline_collection`
...
Processes a single hardcoded PDF. Update the `SOURCE_FILE_TO_ADD` constant at the top of the script to point to your file.

```bash
python single_pdf_parsing_pipeline.py
```

- Collection: `single_pdf_pipeline_collection`
- ChromaDB path: `./chroma_db`
...
| `normalize_elements(elements)` | Flattens chunked elements to a stable dict schema |
...
### `single_pdf_parsing_pipeline.py`
...
| `chunk_elements(elements)` | Splits elements into chunks anchored at title/heading boundaries |
| `normalize_elements(elements)` | Flattens chunked elements to a stable dict schema |
| `generate_embeddings(normalized_chunks)` | Loads the SentenceTransformer model, encodes all chunks, and returns embeddings + model |
| `initialize_chromadb_and_store_chunks(normalized_chunks, embeddings, source_file)` | Creates/opens the ChromaDB collection and inserts all chunks with metadata |
| `query_chromadb(collection, model, question)` | Runs a semantic search and prints top-5 results |
...
| `SOURCE_FILE_TO_ADD` | *(single_pdf_parsing_pipeline.py only)* | Path to the single PDF to process |
